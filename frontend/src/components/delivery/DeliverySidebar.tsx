...
const navGroups = [
  {
    label: 'Deliveries',
    items: [
      { name: 'My Deliveries', href: '/delivery', icon: Bike },
      { name: 'History', href: '/delivery/history', icon: History },
    ],
  },
];

export default function DeliverySidebar({ onCloseMobile }: DeliverySidebarProps) {
          ...
          {navGroups.map((group) => (
            <div key={group.label} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {group.items.map((item) => {
                const isActive = item.href === '/delivery'
                  ? pathname === '/delivery'
